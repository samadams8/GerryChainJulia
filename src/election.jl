...
in a given plan.
"""
function seats_won(name::String, election::Election, party::String)::PlanScore
...
"""
function mean_median(name::String, election::Election, party::String)::PlanScore
    function score_fn(args...)
...
"""
function efficiency_gap(name::String, election::Election, party::String)::PlanScore
    function score_fn(args...)
        ...
        if length(election.parties) != 2
            throw(
                ArgumentError("Efficiency gap is only valid for elections with 2 parties."),
            )
        end
...
function ElectionTracker(
    election::Election,
    scores::Array{S,1} = AbstractScore[],
)::CompositeScore where {S<:AbstractScore}
